...
def _mock_rails(colang_content: str = "some ordinary generated reply") -> MagicMock:
    # src/guardrails/gates.py never imports a client directly (it only
    # imports generate_planner from src.providers.llm, which is where the
    # provider clients actually live). check_safety() always calls
    # _get_rails() first too, so that also needs mocking or every test
    # would make a real network call to build NeMo Guardrails and hit Groq.
    # generate_planner's primary provider is Groq (see src/providers/llm.py),
    # so groq_client is what needs mocking here, not nim_client.
    rails = MagicMock()
